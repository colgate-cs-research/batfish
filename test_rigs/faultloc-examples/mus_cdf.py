...
def main():
    ...
    masterfile_name = "mcs_analyze.csv"
    ...
    writer.writerow(["network", "scenario", "min_mus", "min_f_mus"])
    ...
    for id_filename in sorted(os.listdir(snapshot_ids_dir)):
        ...
        for i, num in enumerate(nums):
            ...
            (faulty_preds, snapshot_candidates, num_mus, num_failures,
                    num_policies, min_mus, min_f_mus) = process_snapshot_num(experiment_name,
            ...
            if (not args.print_per_policy and
                ...
                writer.writerow([network, snapshot_name, min_mus, min_f_mus])
...
def process_snapshot_num(experiment_name, network, snapshot, snapshot_dir,
    ...
    for filename in sorted(os.listdir(output_dir)):
        if filename.startswith('mus_bulk'):
            ...
            policy, candidates, num_mus, min_mus, min_f_mus = (process_mus_intersect(mus_path, num)
                    if args.i_fail else process_mus(mus_path, writer, faulty_preds, num))
    ...
    if not policy_to_candidates: # if no violations:
        no_faults = set()
        return no_faults, snapshot_candidates, 0, 0, 0, 0, 0
    ...
    return (faulty_preds, snapshot_candidates,
            min([min(mus_count) for mus_count in policy_to_muscount.values()]),
            max(policy_to_failcount.values()), len(policy_to_candidates), min_mus, min_f_mus)
...
def process_mus(mus_file_path, writer, faulty_preds, num=10**6):
    preds= set()
    count = 0
    min_mus = 0
    min_f_mus = 0
    with open(mus_file_path, "r") as mus_file:
        for mus in mus_file:
            ...
            if len(preds_list) < min_mus or min_mus == 0:
                min_mus = len(preds_list)
            for x in preds_list:
                if x in faulty_preds:
                    if len(preds_list) < min_f_mus or min_f_mus ==0 :
                        min_f_mus = len(preds_list)
                    break
                preds.add(x)
            if count == (num+1):
                break
    return policy,preds,count-1, min_mus, min_f_mus
