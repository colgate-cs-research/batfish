...
def main():
    ...
    parser.add_argument('-n', '--max number of mcs', dest='num_mcs', action='store',required = True, help='The maximum number of mcs')
    ...
    for filename in sorted(os.listdir(output_dir)):
        if filename.startswith('mus_bulk'):
            ...
            policy, candidates, num_mus = (process_mus_intersect(mus_path, num)
                    if args.i_fail else process_mus(mus_path, args, num))
...
def process_mus(mus_file_path, args, num=10**6):
    preds= set()
    count = 0
    with open(mus_file_path, "r") as mus_file:
        for mus in mus_file:
            ...
            if len(preds_list) <= int(args.num_mcs):
                for x in preds_list:
                    preds.add(x)
            if count == (num+1):
                break
